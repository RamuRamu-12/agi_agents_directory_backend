...
class PostgreSQLDB:
    ...
    def add_agent(self, name: str, description: str, category: str, industry: str, pricing_model: str, accessory_model: Optional[str], website_url: str, tagline: Optional[str] = None, likes: int = 0, overview: Optional[str] = None, key_features: Optional[List[str]] = None, use_cases: Optional[List[str]] = None, created_by: Optional[str] = None, access: Optional[str] = None, tags: Optional[List[str]] = None, preview_image: Optional[str] = None, demo_video: Optional[str] = None) -> Optional[int]:
        conn = self.connect()
        if conn is not None:
            try:
                with conn.cursor() as cursor:
                    query = """
                    INSERT INTO agents (
                        name, description, category, industry, pricing_model, accessory_model, website_url, 
                        tagline, likes, overview, key_features, use_cases, created_by, access, tags, 
                        preview_image, demo_video
                    ) 
    ...
    def update_agent(self, agent_id, name, description, category, industry, pricing_model, accessory_model, website_url, tagline=None, likes=0, overview=None, key_features=None, use_cases=None, created_by=None, access=None, tags=None, preview_image=None, demo_video=None):
        try:
            conn = self.connect()
            if conn is not None:
                cursor = conn.cursor()
                ...
                SET name = %s, description = %s, category = %s, industry = %s, pricing_model = %s, accessory_model = %s, 
                    website_url = %s, tagline = %s, likes = %s, overview = %s, key_features = %s, 
